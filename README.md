...
* [PEP8Online](http://pep8online.com/).
    - The project contains numerous Python files, some generated by Django and a dozen files I authorted myself. At the time of writing this readme, the Python code in this project contains several lines which exceed the recommended max length of 79 characters. I chose to leave them in because breaking them up resulted in less readability. The PEP8 guide states that its guidelines could be bent on an individual basis when doing so raised readability, hence I chose to do so here. Apart from the occasional line that breaks 79 characters, the code is fully PEP8-compliant.

### Unit Testing <a name="code-testing"></a>

Unit tests were written with the Python unittest package.

Currently, about 45% of the code has been covered, according to coverage. This is below the recommended average of 70%. However, due to time constraints, I decided to test most of the views manually. All models and forms, however, have been tested with unittests.
...
The project was developed locally with Visual Studio Code. Environment variables, including secret key, Stripe keys and Database URL, were saved in a settings.json file which was added to .gitignore. For the deployed version, as mentioned in the deployment writeup above, environemnt variables were set in the Config Variables.
